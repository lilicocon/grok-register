...
def _extract_code(content: str) -> Optional[str]:
    if not content:
        return None
    # Grok/x.ai format: XXX-XXX — only match when content looks like a Grok/xAI email
    if re.search(r"(?i)(grok|x\.ai|xai|accounts\.x|verify|verification code)", content):
...
class MailProvider(ABC):
    ...
    def wait_for_otp(
        ...
        """轮询获取6位验证码，超时返回空字符串。exclude_codes: 跳过已知的旧验证码"""
    ...
    def delete_mailbox(
        ...
        try:
            with _build_session(proxy, proxy_selector) as session:
                ...
                logger.warning("MoeMail 删除邮箱失败: status=%s", resp.status_code)
                return False
        except Exception as exc:
            logger.warning("MoeMail 删除邮箱异常: %s", exc)
            return False
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        try:
                            ...
                            time.sleep(3)
                            continue
                        messages = data.get("results") or [] if isinstance(data, dict) else (data if isinstance(data, list) else [])
                        for msg in messages:
                            if not isinstance(msg, dict):
                                continue
                            ...
                            if not content and msg.get("raw"):
                                try:
                                    ...
                                    parsed = _email_mod.message_from_string(msg["raw"], policy=_email_policy.default)
                    ...
                    logger.warning("Cloudflare Temp Email 轮询验证码失败: %s", exc)
                time.sleep(3)
        return ""
...
class YydsMailProvider(MailProvider):
    """YYDS Mail 临时邮箱提供商 (https://maliapi.215.im)"""

    def __init__(self, api_base: str = "https://maliapi.215.im", api_key: str = "", domain: str = ""):
        self.api_base = api_base.rstrip("/")
        self.api_key = api_key
        self.domain = str(domain).strip()
        ...
        with _build_session(proxy, proxy_selector) as session:
            try:
                ...
                if resp.status_code in (200, 201):
                    ...
                    account_id = str(result.get("id") or "").strip()
                    if address and token:
                        return address, f"{token}|{account_id}"
            except Exception as exc:
                logger.warning("YYDS Mail 创建邮箱失败: %s", exc)
        return "", ""
    ...
    def wait_for_otp(
        ...
        _exclude = exclude_codes or set()
        parts = str(auth_credential or "").split("|", 1)
        token = parts[0].strip()
        if not token:
            return ""
        with _build_session(proxy, proxy_selector) as session:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        for msg in messages:
                            ...
                            try:
                                detail_resp = session.get(
                                    f"{self.api_base}/v1/messages/{msg_id}",
                                    headers=self._token_headers(token),
                                    timeout=15, verify=False,
                                )
                                if detail_resp.status_code == 200:
                                    ...
                                    if not content and html_parts:
                                        content = html_parts[0] if isinstance(html_parts, list) else str(html_parts)
                                    code = _extract_code(content)
                                    if code and code in _exclude:
                                        continue
                                    if code:
                    ...
                    logger.warning("YYDS Mail 轮询验证码失败: %s", exc)
                time.sleep(3)
        return ""
    ...
    def delete_mailbox(
        ...
        try:
            with _build_session(proxy, proxy_selector) as session:
                resp = session.delete(
                    f"{self.api_base}/v1/accounts/{account_id}",
                    headers=self._token_headers(token),
                    timeout=15, verify=False,
                )
...
class TempMailLolProvider(MailProvider):
    """TempMail.lol 临时邮箱提供商 (https://api.tempmail.lol/v2)"""

    def __init__(self, api_base: str = "https://api.tempmail.lol/v2", api_key: str = ""):
        self.api_base = api_base.rstrip("/")
            ...
            try:
                ...
                if resp.status_code in (200, 201):
                    if not resp.text.strip().startswith("{"):
                        logger.warning("TempMail.lol API 返回非 JSON（可能 api_base 配置错误）: %s", resp.text[:200])
                        return "", ""
                    data = resp.json()
                    ...
                    logger.warning("TempMail.lol create_mailbox HTTP %s: %s", resp.status_code, resp.text[:200])
            except Exception as exc:
                logger.warning("TempMail.lol 创建邮箱失败: %s", exc)
        return "", ""
    ...
    def wait_for_otp(
        ...
        _exclude = exclude_codes or set()
        token = str(auth_credential or "").strip()
        if not token:
            return ""
        with _build_session(proxy, proxy_selector) as session:
            seen_ids: set = set()
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if data.get("expired") is True:
                            logger.warning("TempMail.lol 邮箱已过期，退出轮询")
                            return ""
                    ...
                    logger.warning("TempMail.lol 轮询验证码失败: %s", exc)
                time.sleep(3)
        return ""
...
def create_provider_by_name(provider_type: str, mail_cfg: Dict[str, Any]) -> MailProvider:
    ...
    elif provider_type == "yydsmail":
        return YydsMailProvider(
            api_base=api_base or "https://maliapi.215.im",
            api_key=str(mail_cfg.get("api_key", "")).strip(),
            domain=str(mail_cfg.get("domain", "")).strip(),
        )
    elif provider_type == "tempmail_lol":
        return TempMailLolProvider(
            api_base=api_base or "https://api.tempmail.lol/v2",
            api_key=str(mail_cfg.get("api_key", "")).strip(),
        )
